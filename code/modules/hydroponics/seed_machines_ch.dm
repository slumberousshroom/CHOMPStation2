...
/obj/machinery/botany/precisioneditor
	...
	circuit = /obj/item/weapon/circuitboard/precisioneditor //chomp edit, makes it buildable 
	...
	"silver",    //chomp pled, getting these chems is fairly rare via exotic seeds and the other way made available by this machine will hopefully be made non viable by increasing the units needed so you have to get mats from mining first. 
	"platinum",  
	"mhydrogen", 
	"steel",  
	"plasteel",  
...
/obj/machinery/botany/precisioneditor/attackby(var/obj/item/O, var/mob/user)
...
/obj/machinery/botany/precisioneditor/proc/add_chem_to_seed(var/chem_name)
  ...
  if(!(chem_name in allowed_reagents))
    ...
    return
  if(chem_amount < 300)
    visible_message("[bicon(src)] Error: A minimum of 300 [SSchemistry.chemical_reagents[chem_name]] is required to perform this action.") //chomp edit, increased the unit cost to 300 to make it so you need a miner to make a plant make certain chems without using exotic seed RNG
    ...
    if(!isnull(SSplants.seeds[seed.seed.name])) //diverge the seed into a new species
      ...
      seed.seed_type = seed.seed.name
      seed.update_seed()
    var/product_amount = 1 + round((chem_amount - 300) / 25) //would rather that it round down, but no proc for that  //chomp edit mostly left this in in the event bigger beakers are added.
